...
復古像素風：硬邊、無圓角、不做模糊陰影，整個 UI 只用一個深色描邊（`--line`）。
馬桶的輪廓是量化過的階梯狀 `clip-path`，JS 的 `halfAt()` 用同一條公式算邊界，
所以雞永遠走不到階梯外面 —— **改形狀的時候兩邊要一起改**。

**像素字只給英數**（Press Start 2P）。繁體中文維持一般無襯線字 ——
Google Fonts 上沒有覆蓋率夠的繁中像素字（DotGothic16 缺 雞／餵／牠／你 等字，
「便便雞」會變成兩個像素字加一個非像素字），混用看起來像壞掉而不是風格。

字型用 `display=swap`，抓不到就直接用系統字，不會擋畫面。
